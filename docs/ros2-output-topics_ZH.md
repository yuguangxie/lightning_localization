...
| `/localization/scan` | `sensor_msgs/msg/PointCloud2` | 开启 | 最近一次有效 LidarLoc scan 使用 `/localization/pose` 同源 final pose 变换到 `ros_output.map_frame` 后低频发布 |
...
`/localization/scan` 发布当前 LiDAR scan 的可视化点云，不是原始传感器 topic。当前实现先在 `LidarLoc` 报告 `lidar_loc_valid_ == true` 后缓存最近一次有效 scan，再在 PGO 高频输出路径中使用与 `/localization/pose` 同源的 final `LocalizationResult.pose_` 将该 scan 变换到 `ros_output.map_frame`，默认 `map`。这样 RViz2 中 scan 与 pose 的位置语义一致，同时不会把每次高频 pose 输出都变成高频大点云发布。

scan 发布受 `ros_output.scan_min_period_sec` 节流，默认 `0.5` 秒；`ros_output.scan_publish_only_on_new_scan=true` 时同一帧缓存 scan 最多发布一次；`ros_output.scan_timestamp_tolerance_sec` 默认 `0.3` 秒，用于拒绝时间差过大的 scan/pose 配对。
...
| Topic | Frame | 配置 |
|---|---|---|
| `/localization/scan` | `ros_output.map_frame` | `ros_output.publish_scan`、`ros_output.scan_topic`、`ros_output.scan_pose_source`、`ros_output.scan_min_period_sec`、`ros_output.scan_timestamp_tolerance_sec`、`ros_output.scan_publish_only_on_new_scan` |
