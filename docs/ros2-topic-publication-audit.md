...
| `/localization/scan` | ROS2 topic | `sensor_msgs/msg/PointCloud2` | Added after RViz2 visualization enhancement | `LocSystem::PublishScanCloud()` via `Localization::SetScanCloudCallback()` from the PGO high-frequency output path | Latest valid LidarLoc scan paired with final pose output; throttled by `ros_output.scan_min_period_sec`, timestamp tolerance, and new-scan-only filtering | `ros_output.publish_scan`, `ros_output.scan_topic`, `ros_output.scan_pose_source`, `ros_output.scan_timestamp_tolerance_sec`, `ros_output.scan_publish_only_on_new_scan` | Scan is transformed into `ros_output.map_frame` using the same final pose source as `/localization/pose`; intended for low-rate RViz2 visualization, not raw sensor streaming. |
