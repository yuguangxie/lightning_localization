...
| `/localization/scan` | ROS2 topic | `sensor_msgs/msg/PointCloud2` | RViz2 可视化增强后新增 | `LocSystem::PublishScanCloud()`，经 `Localization::SetScanCloudCallback()` 从 PGO 高频输出路径回调 | 最近一次有效 LidarLoc scan 与 final pose 输出配对；受 `ros_output.scan_min_period_sec`、时间戳容差和只发布新 scan 逻辑节流 | `ros_output.publish_scan`、`ros_output.scan_topic`、`ros_output.scan_pose_source`、`ros_output.scan_timestamp_tolerance_sec`、`ros_output.scan_publish_only_on_new_scan` | scan 使用与 `/localization/pose` 同源的 final pose 变换到 `ros_output.map_frame`，用于低频 RViz2 可视化，不是原始传感器流。 |
