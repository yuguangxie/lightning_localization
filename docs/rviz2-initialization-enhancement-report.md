...
| `/localization/scan` | `sensor_msgs/msg/PointCloud2` | 开启 | 当前 scan 按定位 pose 变换到 `map` frame 后发布 |
...
| scan/map PointCloud2 输出 | OK | `ros_output.publish_scan/map`、`SetScanCloudCallback`、`SetMapCloudCallback`、`/localization/scan`、`/localization/map` |
...
| scan/map display 依赖定位数据流 | `/localization/map` 需要地图 chunk 成功加载；`/localization/scan` 需要有效定位结果 | 人工验证时同时检查 map path、bag topic、TF 和定位状态 |
