...
| `src/core/system/loc_system.cc` | 修改 | `/initialpose`、service、状态输出、diagnostics 集成；核心未初始化时拒绝 pose 应用；发布 `/localization/scan` 和 `/localization/map` |
| `src/core/localization/localization.h/.cpp` | 修改 | 增加 scan/map 点云可视化回调，不改变定位算法 |
| `src/core/localization/lidar_loc/lidar_loc.h/.cc` | 修改 | 在 tiled map chunk 更新时回调当前局部地图点云 |
...
- PointCloud2 Current Scan display: `/localization/scan`，默认开启
- PointCloud2 Local Map display: `/localization/map`，默认开启
...
| `/localization/scan` | `sensor_msgs/msg/PointCloud2` | 开启 | 最近一次有效 LidarLoc scan 按 `/localization/pose` 同源 final pose 变换到 `map` frame 后低频发布 |
| `/localization/map` | `sensor_msgs/msg/PointCloud2` | 开启 | 当前已加载的局部 tiled runtime map，transient local QoS |
...
| scan/map PointCloud2 输出 | OK | `ros_output.publish_scan/map`、`scan_pose_source=final_pose`、`scan_min_period_sec=0.5`、`SetScanCloudCallback`、`SetMapCloudCallback`、`/localization/scan`、`/localization/map` |
| RViz2 scan/map display 默认开启 | OK | `rviz/lightning_localization.rviz` 中 Current Scan 和 Local Map display 启用 |
| RViz2 左右 dock 不隐藏 | OK | `Hide Left Dock: false`、`Hide Right Dock: false`，Views 面板存在 |
...
5. 检查 TF、`/localization/pose`、`/localization/scan` 和 `/localization/map`。
6. 确认左侧 Displays 面板可手动收窄，右侧 Views 面板可打开并切换视角。
7. 使用 service 注入初始位姿。
8. 使用 RViz2 `2D Pose Estimate` 发布 `/initialpose`。
9. 检查 `/localization/initialization_status` 和 `/localization/diagnostics`。
10. 接入真实 bag/map 后确认定位状态是否能进入 `GOOD`，并确认 scan/map 点云随数据流更新。
...
| scan/map display 依赖定位数据流 | `/localization/map` 需要地图 chunk 成功加载；`/localization/scan` 需要有效 LidarLoc scan 和 final pose 时间戳接近 | 人工验证时同时检查 map path、bag topic、TF、定位状态和 `scan_timestamp_tolerance_sec` |
